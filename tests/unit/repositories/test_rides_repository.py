...
def make_ride(
    *, ride_id: str = "r1", user_id: str = "u1", vehicle_id: str = "v1"
) -> Ride:
    return Ride(
        ride_id=ride_id,
        user_id=user_id,
...
def test_add_and_get_by_id_and_user_id() -> None:
    ...
    r1 = make_ride(ride_id="r1", user_id="u1", vehicle_id="v1")
    r2 = make_ride(ride_id="r2", user_id="u1", vehicle_id="v2")
...
def test_snapshot_and_restore_round_trip_preserves_order_and_status() -> None:
    ...
    active = make_ride(ride_id="r_active", user_id="u1", vehicle_id="v1")

    cancelled = make_ride(ride_id="r_cancelled", user_id="u1", vehicle_id="v2")
    cancelled.cancel()

    completed = make_ride(ride_id="r_done", user_id="u2", vehicle_id="v3")
    ...
    r1 = restored.get_by_id("r_active")
    assert r1 is not None
    assert r1.status() == RideStatus.ACTIVE

    r2 = restored.get_by_id("r_cancelled")
    assert r2 is not None
    assert r2.status() == RideStatus.CANCELLED

    r3 = restored.get_by_id("r_done")
    ...
    assert [r.ride_id for r in restored.get_by_user_id("u2")] == ["r_done"]


def test_restore_clears_previous_state() -> None:
    repo = RidesRepository()

    repo.add(make_ride(ride_id="old", user_id="u1", vehicle_id="v1"))
    new_ride = make_ride(ride_id="new", user_id="u2", vehicle_id="v2")

    repo.restore({new_ride.ride_id: _ride_to_dict(new_ride)})

    assert repo.get_by_id("old") is None
    assert repo.get_by_user_id("u1") == []
    assert repo.get_by_id("new") is not None
    assert [r.ride_id for r in repo.get_by_user_id("u2")] == ["new"]


def test_ride_from_dict_cancelled_with_end_time_raises() -> None:
    data = _ride_to_dict(make_ride(ride_id="r1", user_id="u1", vehicle_id="v1"))
